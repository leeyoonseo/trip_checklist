...
var TripCheckList = (function(){
    ...
    console.log(localStorage.getItem(CHECK_LIST_DATA))
    ...
    saveBtn.addEventListener('click', function(){
        myListArr.sort(function(a, b){
            return a.name < b.name ? -1 : a.name > b.name ? 1 : 0;
        });

        for(var i = 0; i < myListArr.length; i++){
            for(var j = 0; j < data.length; j++){
                if(data[j].id === myListArr[i].id){
                    data[j].checked = "true";
                }else{
        ...
        console.log(data)
        localStorage.setItem(CHECK_LIST_DATA, JSON.stringify(data));
    });
